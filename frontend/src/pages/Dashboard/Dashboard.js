...
const Dashboard = () => {
  ...
  async function fetchDevices() {
    try {
      ...
      // ✅ เช็ค status ก่อน .json() เพื่อป้องกัน "Unauthorized is not valid JSON"
      if (resp.status === 401) {
        localStorage.removeItem('base-shell:auth')
        navigate('/signin')
        return
      }
                    ...
                    <CardContent sx={{ textAlign: 'center', p: 3, pt: 4 }}>
                      ...
                      <Box sx={{ mt: 2 }}>
                        <Button
                          ...
                          size="small"
                          fullWidth
                          onClick={(e) => { e.stopPropagation(); navigate(`/dashboard/${device._id}/view`) }}
                        >
                          View Dashboard
                        </Button>
                      </Box>
